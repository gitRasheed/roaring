...
// bitmapOrBulkMergeFixture holds two variants of a left/right bitmap pair so
// benchmarks can alternate between them and avoid mutating a single receiver
// across iterations. The fixtures below exercise the in-place Bitmap.Or bulk
// merge path (many interleaved source-only keys).
