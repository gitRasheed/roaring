...
// checkDifference checks both shipping buffer contracts: zero-length buffer
// with exact capacity and canaries (andNotArray) and a buffer aliasing set1
// in place (iandNotArray).
...
// Skewed sizes drive long retained-block runs and the spill/drain handoff.
func TestAndnotNEONSkewSpill(t *testing.T) {
	...
	// sparse pick-off: b deletes scattered singles across many a blocks
	a := make([]uint16, 512)
	for i := range a {
		a[i] = uint16(i * 3)
	}
...
// AndNot of a container with itself reaches the kernel with set1, set2 and
// buffer all sharing one backing array; the correct result is empty.
func TestAndnotNEONSelfAlias(t *testing.T) {
	...
	for _, n := range []int{16, 65, 512} {
		...
		// same values, separate arrays, fresh buffer
...
// Adjacent duplicates pass upstream Validate; results are unspecified but
// every store must stay within the buffer's capacity.
